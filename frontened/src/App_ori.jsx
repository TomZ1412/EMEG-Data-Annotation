...
const API_BASE_URL = '10.140.37.162:10000'
...
function App() {
      ...
      .then(annotationResponse => {
        if (annotationResponse.data) {
          const annotationData = {
            bad_channels: annotationResponse.data.bad_channels || [],
  ...
  const handleAnnotate = () => {
    ...
    setLoading(true);
    axios.post(`http://${API_BASE_URL}/api/annotate`, {
      file_path: selectedFile,
      bad_channels: badChannels,
      user: currentUser,
      discarded: isDataDiscarded, // 添加数据弃用状态
      sub_block_index: subBlockIndex // 添加子图索引
    }).then((response) => {
  ...
  const handleNextUnannotated = () => {
    ...
    setLoading(true);
    axios.get(`http://${API_BASE_URL}/api/next_unannotated?user=${currentUser}`)
      .then(res => {
        if (res.data.file_path) {
          setSelectedFile(res.data.file_path);
          setSubBlockIndex(0); // 重置子图索引
          setError(null);
          // 注意：badChannels 和 isDataDiscarded 会在文件切换时从缓存或后端重新加载
        } else {
  ...
  const handleFileSelect = async (filePath) => {
    ...
    setSubBlockIndex(0); // 重置子图索引
    // 注意：badChannels 和 isDataDiscarded 会在 useEffect 中从缓存或后端重新加载
  };

  // ✅ 处理子图选择
  const handleSubBlockSelect = (index) => {
    setSubBlockIndex(index);
    // 注意：这里不需要手动调用 fetchVisualizationData，
    // 因为 useEffect 会监听 subBlockIndex 的变化自动重新获取波形数据
    // 但标注信息（badChannels 和 isDataDiscarded）保持不变
  };
          ...
          <>
            ...
            <div style={{ flex: 1, minHeight: 0 }}>
              <WaveformViewer
                ...
                setBadChannels={handleBadChannelsChange} // 使用新的处理方法
                loading={loading}
                // onDataDiscardedChange={handleDataDiscardedChange}
